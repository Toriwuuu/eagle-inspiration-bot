...
>
> **擔心安全嗎？** 先看這節 → [安全性與隱私（先看這個再裝）](#安全性與隱私先看這個再裝)。一句話：全部在你自己電腦跑、不收集任何資料、程式碼全公開可驗證。
>
> **裝之前一定要懂的風險** → [使用前需知與風險](#使用前需知與風險)。簡單講：自動抓取通常違反這些網站的服務條款，**帳號可能被封、IP 可能被擋**，請自行評估再用。
...
要把別人寫的東西裝進自己電腦，會擔心很正常。這節用白話講清楚——
**而且每一條你都能自己驗證，不用「相信我」。**

### 這個工具不會做的事

- **不會上傳你的 Eagle 收藏**到任何地方。你的圖庫永遠只在你電腦裡。
- **不會收集、不會回傳任何資料**。沒有分析、沒有追蹤、沒有「偷偷回報」。
- **看不到你的密碼**。Mobbin 是「你自己」在一個真的瀏覽器視窗裡用 Google 登入，
  工具只保留登入後的 session 存在你本機（`~/.eagle-bot/`），從頭到尾碰不到你的帳號密碼。
- **沒有伺服器、不用註冊、沒有帳號**。它整個就跑在你自己這台 Mac 上。

### 它只會連這幾個地方

跑起來時，它對外只會連：

- **你自己的電腦**（`localhost` / `127.0.0.1`）— 跟你本機的 Eagle、控制台溝通用
- **這 4 個設計網站**：`awwwards.com`、`mobbin.com`、`godly.website`、`land-book.com` — 就是它去抓靈感的來源
...
- **程式碼 100% 公開**，就在 GitHub 上，主程式是單一一個 `bot.js`，
  沒有壓縮、沒有混淆、沒有看不懂的編譯產物——任何工程師十分鐘可以掃完。
- **只用 1 個外部套件**：[Playwright](https://playwright.dev)（微軟維護的瀏覽器自動化工具，業界標準）。
  依賴越少，能被動手腳的空間越小。
- **MIT 授權**，可自由使用、修改、檢查。
- 想自己確認「它到底連去哪」，在 `bot.js` 搜尋 `fetch`、`goto` 就能看到全部對外連線。

> **不會看程式碼？** 完全沒關係。最實在的做法是：
> **找一個你信任、會寫程式的朋友，把 `bot.js` 丟給他掃一遍**再決定要不要裝。
> 我把這句話寫在這裡，就是因為這個專案不怕被檢查。

### 你該知道的兩件事（誠實說）

- **它會用瀏覽器去抓那 4 個網站的內容**。這是給「個人收集靈感」用的，請斟酌自己的使用情境，別拿去做大量商業用途。
- **每週自動排程是「選用」的**，你不裝就不會有。裝了之後隨時可以用 `./uninstall-schedule.sh` 一鍵移除。

---

## 使用前需知與風險

> 上一節「安全性與隱私」講的是**這支工具對你電腦安不安全**。
> 這一節講的是**「使用這種自動抓取工具」這件事本身的風險**——這跟工具寫得好不好無關，
> 是所有 scraping 行為共同的代價。**請讀完再決定要不要用。**

### 1. 自動抓取通常違反網站服務條款（ToS）

Awwwards、Mobbin、Godly、Land-book 這類網站，服務條款多半**禁止自動化抓取**。
這個工具做的就是自動抓取。雖然它有內建一些「對網站比較客氣」的設計（見下方），
但**本質上仍可能違反這些網站的使用條款**，要不要承擔這個風險，由你自己決定。

**這不是危言聳聽，是 Mobbin 官方自己白紙黑字寫的。** 以下為官方條款原文：

> **禁止自動抓取**（[Mobbin Terms of Service](https://mobbin.com/terms)）：
> *"unauthorised use of any scraper, robot, bot, spider, crawler or any other automated device or means to access, acquire, copy or monitor any portion of the Website..."*
>
> 中譯：未經授權使用任何 scraper、robot、bot、spider、crawler 或其他自動化方式，存取、取得、複製或監控本網站的任何部分。

> **違反可不經通知直接終止帳號**（[Mobbin Acceptable Use Policy](https://mobbin.com/acceptable-use)）：
> *"...terminate... the account itself or take any remedial or preventative action it deems appropriate, without notice."*
>
> 中譯：Mobbin 保留權利，可在不事先通知的情況下直接終止帳號、或採取它認為適當的補救或預防措施。

> 補充：條款另也禁止用任何自動化工具（含 AI / 機器學習）對其內容做衍生、建立競品或商業用途。

要釐清的是：這是**違反 Mobbin 的契約條款**，不等於「違法犯罪」——但 Mobbin 有權直接停你帳號。

### 2. 帳號可能被封禁（主要是 Mobbin）

Mobbin 需要登入才能抓。如果**抓的頻率太高、單次量太大**，Mobbin 有可能偵測到異常行為，
導致你的 **Mobbin 帳號被暫時或永久停權**。

降低風險的做法：

- **不要用你很重要、或付費很久的主帳號**。可以考慮另開一個帳號專門給它用。
- **維持低頻率**：預設「每週一次」就是刻意保守的設定，**不要為了多抓而改成每天/每小時**。
- **單次量別衝太高**：`config.json` 裡的 `maxSites` / `maxApps` 等數字維持小一點。
- 真的不放心，就**不要登入 Mobbin**，只用另外三個免登入來源（Awwwards / Godly / Land-book）。

### 3. IP 可能被擋（主要是 Land-book / Cloudflare）

部分網站（例如 Land-book）用 Cloudflare 之類的防禦系統。短時間大量請求，
有可能讓**你目前的網路 IP 被暫時擋下**（網頁打不開、出現人機驗證）。
通常停一段時間會自己恢復；遇到時請**先停手、隔一陣子再用**，不要一直重試硬衝。

### 4. 內容是別人的，僅供「個人靈感參考」

抓下來的設計作品、截圖、影片，**著作權都屬於原作者與原網站**。
這個工具的定位是**幫你個人收集靈感、做設計研究**，
**不可**拿去再散布、商用、二次發布或宣稱為自己的作品。

### 5. 它隨時可能突然失效

抓取是「貼著網站當下的網頁結構」運作的。只要對方改版，
某個來源就可能**突然抓不到或抓錯**——這是這類工具的常態，不是壞掉。
它對你電腦不會造成傷害，只是那個來源要等修。

---

### 一句話總結

這是一個**給個人做設計研究用**的小工具。它已經盡量「對網站客氣」
（模擬人類滑動、自動跳過抓過的、預設一週只跑一次），
但**無法消除上面的風險，只能降低**。
請用保守的設定、合理的頻率、可以承受被封的帳號——**風險與責任由使用者自負**。
